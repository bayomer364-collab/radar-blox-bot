    ...
    client.on('interactionCreate', async interaction => {
        ...
        if (interaction.isChatInputCommand() && interaction.commandName === 'guide') {
            ...
            modal.addComponents(
                ...
                new ActionRowBuilder().addComponents(
                    new TextInputBuilder().setCustomId('g1Name').setLabel('1. Guide Adı').setStyle(TextInputStyle.Short).setRequired(true)
                ),
                new ActionRowBuilder().addComponents(
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'guideModal1') {
                ...
                modal2.addComponents(
                    new ActionRowBuilder().addComponents(
                        new TextInputBuilder().setCustomId('g2Name').setLabel('2. Guide Adı').setStyle(TextInputStyle.Short).setRequired(true)
                    ),
                    new ActionRowBuilder().addComponents(
                        new TextInputBuilder().setCustomId('g2Text').setLabel('2. Guide İçeriği').setStyle(TextInputStyle.Paragraph).setRequired(true)
                    ),
                    new ActionRowBuilder().addComponents(
                        new TextInputBuilder().setCustomId('g3Name').setLabel('3. Guide Adı').setStyle(TextInputStyle.Short).setRequired(true)
                    ),
                    new ActionRowBuilder().addComponents(
                ...
                const selectMenu = new StringSelectMenuBuilder()
                    .setCustomId(`guide_select_${Date.now()}`)
                    .setPlaceholder('Bir rehber (guide) seçin...')
                    .addOptions([
                        { label: data.g1Name, description: `${data.g1Name} detayını görüntüle`, value: data.g1Name },
                        { label: g2Name, description: `${g2Name} detayını görüntüle`, value: g2Name },
                        { label: g3Name, description: `${g3Name} detayını görüntüle`, value: g3Name }
                    ]);
                ...
                client.guideStorage = client.guideStorage || {};
                client.guideStorage[selectMenu.data.custom_id] = guides;

                return await interaction.reply({ content: 'Rehber başarıyla oluşturuldu!', ephemeral: true });
            }
        }

        // 3. Menüden seçim yapıldığında
        if (interaction.isStringSelectMenu() && interaction.customId.startsWith('guide_select_')) {
            ...
            const content = guides ? guides[selectedValue] : 'Rehber bulunamadı.';
